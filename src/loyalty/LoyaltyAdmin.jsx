...
//
// The tab is a small customer-management hub, split into three sections:
//   • Program — the original loyalty admin, matching the customer loop in
//     the bae-loyalty-app repo:
//       1. Set the punch-card economics (reward_config: threshold + reward
//          text). The customer app reads this row directly via Supabase RLS,
//          so Save here changes what every customer sees.
//       2. Fulfill rewards: when a customer's badges cross the threshold, the
//          claim RPC creates a reward_redemptions row with a code the
//          customer shows staff. Verify the code on their screen, hand over
//          the reward, mark it fulfilled here.
//   • Content — news + growing tips for the app feed (ContentManager.jsx,
//     customer_content table, migration 0035).
//   • Customers — the customer roster with aggregate counts
//     (CustomersPanel.jsx).
//
// Tables ship via migrations 0034_bae_loyalty.sql + 0035_bae_customer_hub.sql
// (manual apply); until then the API answers `unavailable` and each section
...
export function LoyaltyAdmin({ showToast }) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex gap-1 bg-gray-100 rounded-lg p-1 w-fit">
        {SECTIONS.map(s => (
          <button
            ...
            className={`px-3 py-1 text-sm rounded-md ${section === s.id
      ...
      {section === 'content' && <ContentManager showToast={showToast} />}
