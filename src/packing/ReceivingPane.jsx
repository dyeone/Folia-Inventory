...
// New-order receiving at the packing table. Admin uploads/marks a purchase
// order as "ordered"; this pane lists its species lines so the packer can
// label + count the physical shipment:
//
...
export function ReceivingPane({ printDest, showToast, onClose, onChanged }) {
  ...
  const [varietyById, setVarietyById] = useState(new Map());
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setSpeciesById(new Map((species || []).map(s => [s.id, s])));
        setVarietyById(new Map((varieties || []).map(v => [v.id, v])));
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-5 py-3">
          <div className="max-w-3xl mx-auto space-y-2">
            ...
            {pos === null ? (
              ...
              <div className="text-center text-gray-500 py-12">No incoming orders right now.</div>
            ) : (
              pos.map(po => (
                <button
                  key={po.id}
