...
export function NewBoxModal({ inventoryItems, onClose, onRefreshItems, showToast }) {
  ...
  const fillFromHistory = (h) => {
    ...
    setForm(prev => ({
      ...
      // shipmentMethod deliberately NOT inherited: the form has no field for
      // it, so a stale "UPS Next Day" from the old box would ride invisibly
      // onto every item and can contradict the visible carrier toggle.
    }));
  };
        ...
        <div className="border-b border-gray-200 px-4 sm:px-5 py-3 sm:py-4 flex items-center justify-between flex-shrink-0">
          <div className="min-w-0 flex-1">
            <h3 className="font-semibold text-gray-900 text-base sm:text-lg flex items-center gap-2">
              ...
              {phase === 'form' ? 'Add shipment' : `Box ${shortBoxCode(boxId)}`}
            </h3>
                ...
                {hMatches.length === 0 ? (
                  ...
                  >
                    ...
                    <div className="text-xs text-gray-500 truncate">
                      ...
                      {/* Last-shipped date — an old date is the cue that this
                          address may be stale; double-check before shipping. */}
                      {h.at && <span className="text-gray-400"> · {new Date(h.at).toLocaleDateString()}</span>}
                    </div>
                  </button>
                ))}
              </div>
...
function ScanPanel({ boxId, form, addedItems, inventoryItems = [], onAddItem, manualSku, setManualSku, onManualSubmit, onOpenScanner, onDone }) {
  ...
  // Just-added items STAY in the results (rendered as a disabled "Added"
  // row) — removing them would shift the rows up mid-double-click and land
  // the second tap on the wrong plant's Add button.
  const matches = ql
          ...
          matches.length === 0 ? (
            ...
            <>
              {matches.map(i => {
                ...
                return (
                  <div key={i.id} className={`flex items-center gap-2 px-3 py-2 border rounded-lg ${added ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-gray-200'}`}>
                    ...
                    {added ? (
                      <span className="shrink-0 inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1.5 rounded-lg bg-emerald-100 text-emerald-700">
